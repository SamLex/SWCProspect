...
/** The worker for the edit deposit page for SWCProspect. */
...
$cleanDepositID = filter_input(INPUT_POST, 'depositid', FILTER_SANITIZE_NUMBER_INT);
...
if ($db->init() === true) {
    ...
    if (is_int($validSize) && is_int($validLocX) && is_int($validLocY) && is_int($validPlanetID) && is_int($validType) && is_int($validDepositID)) {
        ...
        if (!is_null($type) && !is_null($planet)) {
            // Create new deposit instance and save to db, causing update of existing
            $deposit = new Deposit($validSize, $validLocX, $validLocY, $planet, $type, $db, $validDepositID);
            $deposit->save();
        }
    }
}
